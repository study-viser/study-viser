...
const COURSE_OPS = [
  ...
  'enrollStudent', 'unenrollStudent', 'getSecretCode', 'teachCourse',
] as const;
...
function CourseActionForm({ op, courses, users, onSuccess }: {
      ...
      switch (op) {
        case 'createCourse':
          await createCourse({
            ...
            instructorId: f.get('instructorId') as string || undefined,
            listingId:   f.get('listingId') as string || undefined,
          });
          break;
        ...
        case 'updateCourse':
          await updateCourse(parseInt(f.get('crn') as string, 10), {
            ...
            instructorId: f.get('instructorId') as string || undefined,
            listingId:    f.get('listingId') as string || undefined,
            externalURLs: f.get('externalURLs')
              ? (f.get('externalURLs') as string).split('\n').map(s => s.trim()).filter(Boolean)
              : undefined,
          });
          break;
  ...
  const userOptions       = users.map(u => ({ value: u.id, label: `${u.name} — ${u.email}` }));
  const instructorOptions = users.filter(u => u.role === 'INSTRUCTOR').map(u => ({ value: u.id, label: `${u.name} — ${u.email}` }));

  return (
    <Form onSubmit={handleSubmit} className="p-3 border rounded bg-light">
      <FeedbackAlert feedback={feedback} onClose={() => setFeedback(null)} />

      {/* CRN selector for update/delete/unenroll/getSecretCode/teachCourse */}
      {(op === 'updateCourse' || op === 'deleteCourse' || op === 'unenrollStudent' || op === 'getSecretCode' || op === 'teachCourse') && (
      ...
      {/* Instructor selector for teachCourse */}
      {op === 'teachCourse' && (
        <SelectField name="instructorId" label="Select Instructor" options={instructorOptions} required />
      )}
      ...
      {(op === 'createCourse' || op === 'updateCourse') && (
        <Row>
          ...
          <Col md={6}>
            <SelectField name="instructorId" label="Instructor (optional)" options={instructorOptions} />
          </Col>
          <Col md={6}><TextField name="listingId" label="Listing Code (optional)" placeholder="e.g. ICS 314" /></Col>
          {op === 'updateCourse' && (
            <Col md={12}>
              <TextAreaField name="externalURLs" label="External URLs (one per line)" placeholder="https://example.com" rows={3} />
            </Col>
          )}
        </Row>
      )}
      ...
      switch (op) {
        case 'createTerm':
          await createTerm({
            ...
            referenceDefinition: f.get('referenceDefinition') as string || undefined,
            coveredOn:           f.get('coveredOn') ? new Date(f.get('coveredOn') as string) : undefined,
          });
          break;
    ...
    <Form onSubmit={handleSubmit} className="p-3 border rounded bg-light">
      ...
      {(op === 'createTerm' || op === 'updateTerm') && (
        <Row>
          ...
          <Col md={4}><TextField name="coveredOn" label="Covered On" type="date" /></Col>
          <Col md={12}><TextAreaField name="referenceDefinition" label="Reference Definition" placeholder="Instructor reference / context (optional)" rows={3} /></Col>
