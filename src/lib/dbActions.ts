...
//
// Course
//   createCourse(data)               — create a new course (auto-generates secret)
...
export async function createCourse(data: {
  ...
  // secret is auto-generated by the DB via @default(uuid()) — do not pass manually
  title: string;
  description?: string;
  location?: string;
  instructorId?: string;
...
export async function updateCourse(
  ...
  data: Partial<{
    ...
    listingId: string;
    externalURLs: string[];
    // secret is managed by the DB — only update if rotating
    secret: string;
  }>
) {
  try {
...
/** Enroll a student in a course using the course secret */
export async function enrollStudent(secret: string, studentId: string) {
  try {
